<script lang="ts">
	...
	const FALLBACK: Array<{ title: string; author: string; date: string; href: string }> = [
		{
	...
	const items = $derived(
		articles.length >= 5
			? articles.slice(0, 5).map((a) => ({
					...
					date:   new Date(a.publishedAt).toLocaleDateString('de-DE', { day: 'numeric', month: 'short', year: 'numeric' }),
					href:   `/${a.urlPath}`,
					thumb:  a.thumbnail || ''
			  }))
